...
const main = () => {
  const nameArg = process.argv.find((arg) => arg.match(/--username\w*/));
  ...
  rl.on("line", async (line) => {
    ...
    switch (comandName) {
      case "up":
        ...
        rl.setPrompt(
          `\x1b[36mYou are currently in ${currentDirectory} >\n\x1b[0m`
        );
        rl.prompt();
        break;
      ...
      case "ls":
        nwd.list(currentDirectory).then((list) => {
          list = list
            .map((elem) => {
              return {
                Name: elem.name,
      ...
      case "os":
        switch (comandContent) {
          case "--EOL":
            ...
            break;
          case "--cpus":
            const cpus = operating.getCpus();
            console.log(`Overal amount of CPUs is ${Object.keys(cpus).length}`);
            console.table(cpus);
            break;
          case "--homedir":
